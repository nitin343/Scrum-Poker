        ...
        >
            ...
            >
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-[0.05] pointer-events-none">
                    <span className="text-6xl font-bold tracking-[0.5em] text-white">SCRUM</span>
                </div>
                    ...
                    {isScrumMaster && (
                        <div className="flex gap-2">
                            {!areCardsRevealed ? (
                                <button
                                    onClick={onReveal}
                                    disabled={!participants.every(p => p.hasVoted)}
                                    className="px-6 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-full font-bold text-xs uppercase tracking-wider transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Reveal Cards
                                </button>
                            ) : (
                ...
                {seats.map((seat, idx) => {
                    ...
                    return (
                        <motion.div
                            ...
                            style={{
                                perspective: 1000,
                                opacity: phase === 'IDLE' ? 1 : (shouldRenderCard ? 1 : 0) // Show all in IDLE, hide non-voted during animation
                            }}
                        >
...
function computeTableDimensions(count: number) {
    ...
    const w = maxH * CARD_WIDTH + (maxH - 1) * CARD_GAP + 200;
    return { width: Math.max(500, w), height: 250 };
}
