...
const setupSocket = (io) => {
    ...
    const fetchIssueData = async (roomId, issueStub) => {
        try {
            // 1. Fetch from Jira
            let jiraIssue = null;
            try {
                jiraIssue = await JiraService_1.jiraService.getIssue(issueStub.issueKey);
            }
            catch (jiraErr) {
                Logger_1.logger.warn(`Jira fetch failed for ${issueStub.issueKey}`, jiraErr);
            }
            // 2. Fetch History from DB (concurrently?)
            // 2. Fetch History from DB (concurrently?)
            const Sprint = (await Promise.resolve().then(() => __importStar(require('./models/Sprint')))).default;
            const room = (0, store_1.getRoom)(roomId);
            ...
            if (jiraIssue && jiraIssue.fields) {
                const fullIssue = {
                    ...
                    timeEstimate: jiraIssue.fields.timetracking?.originalEstimate // Time Estimate
                };
            ...
            Logger_1.logger.error(`Error fetching data for ${issueStub.issueKey}`, error);
    ...
    const loadAndBroadcastIssue = async (roomId, index) => {
        Logger_1.logger.info('[SOCKET] loadAndBroadcastIssue called', { roomId, index });
        const room = (0, store_1.getRoom)(roomId);
        if (!room) {
            Logger_1.logger.warn('[SOCKET] loadAndBroadcastIssue: Room not found');
            return;
        }
        ...
        if (cached) {
            ...
            Logger_1.logger.info(`Fetching fresh data for ${issueStub.issueKey}`);
            try {
                ...
                Logger_1.logger.error('Failed to fetch fresh issue data', err);
    ...
    const prefetchNext = async (roomId, nextIndex) => {
        const room = (0, store_1.getRoom)(roomId);
        if (!room || nextIndex >= room.totalIssues)
            return;
        ...
        if (cached && (Date.now() - cached.timestamp < 5 * 60 * 1000))
            return;
        Logger_1.logger.debug(`[Prefetch] Warming cache for ${stub.issueKey}`);
    ...
    io.on('connection', (socket) => {
        Logger_1.logger.info('User connected', { socketId: socket.id });
        // JOIN ROOM
        socket.on('join_room', async (payload) => {
            ...
            try {
                ...
                Logger_1.logger.info('[JOIN_ROOM] Processing join request', { roomId, odId });
                if (!roomId || !odId) {
                    Logger_1.logger.error('[JOIN_ROOM] Missing required fields', { roomId, odId });
                    socket.emit('error', { message: 'Missing room or user ID' });
                    return;
                }
                let room = (0, store_1.getRoom)(roomId);
                if (!room) {
                    if (isScrumMaster) {
                        Logger_1.logger.info('[JOIN_ROOM] Creating new room:', { roomId, odId });
                        room = (0, store_1.createRoom)(roomId, odId, socket.id, roomName || 'Scrum Poker', {
                            boardId, sprintId, sprintName, companyId, displayName
                        });
                    }
                    else {
                        Logger_1.logger.error('[JOIN_ROOM] Room does not exist and user is not scrum master');
                        socket.emit('error', { message: 'Room does not exist' });
                        return;
                    }
                }
                else {
                    Logger_1.logger.info('[JOIN_ROOM] Joining existing room, setting participant with odId:', odId);
                    // Repair/Update Room Context if Admin provides it (Sticky Session Fix)
                    if (isScrumMaster && (boardId || sprintId)) {
                        if (boardId)
                            room.boardId = boardId;
                        if (sprintId)
                            room.sprintId = sprintId;
                        if (sprintName)
                ...
                socket.join(roomId);
                Logger_1.logger.info('[JOIN_ROOM] User joined room. Participants now:', Array.from(room.participants.keys()));
                ...
                if (room.issues.length > 0) {
                    loadAndBroadcastIssue(roomId, room.currentIssueIndex).catch(err => Logger_1.logger.error('Initial issue load failed', err));
                }
            }
            catch (error) {
                Logger_1.logger.error('[JOIN_ROOM] Error:', error);
        ...
        socket.on('join_as_guest', async ({ sessionId, displayName }) => {
            try {
                ...
                if (!session) {
                    socket.emit('error', { message: 'Invalid Link' });
                    return;
                }
                ...
                if (!room) {
                    ...
                    room = (0, store_1.createRoom)(roomId, '', '', session.sprintName, {
                ...
                const guest = (0, store_1.addGuestToRoom)(roomId, guestId, socket.id, displayName);
                if (!guest)
                    return;
                socket.join(roomId);
                ...
                if (room.issues.length > 0) {
                    loadAndBroadcastIssue(roomId, room.currentIssueIndex).catch(err => Logger_1.logger.error('Guest issue load failed', err));
                }
            }
            catch (e) {
                Logger_1.logger.error('Error in join_as_guest', e);
        ...
        socket.on('set_issues', ({ roomId, issues }) => {
            try {
                ...
                (0, store_1.setRoomIssues)(roomId, stubs);
                ...
                Logger_1.logger.error('[SET_ISSUES] Failed', err);
            }
        });
        // NEXT ISSUE
        socket.on('next_issue', async ({ roomId }) => {
            try {
                ...
                Logger_1.logger.error('[NEXT_ISSUE] Error:', error);
        ...
        socket.on('prev_issue', async ({ roomId }) => {
            try {
                ...
                Logger_1.logger.error('[PREV_ISSUE] Error:', error);
        ...
        socket.on('select_card', ({ roomId, odId, card }) => {
            Logger_1.logger.info('[SELECT_CARD] Received:', { roomId, odId, card });
            const room = (0, store_1.getRoom)(roomId);
            if (!room) {
                Logger_1.logger.error('[SELECT_CARD] Room not found:', roomId);
                return;
            }
            Logger_1.logger.info('[SELECT_CARD] Room participants:', Array.from(room.participants.keys()));
            const p = room.participants.get(odId);
            if (p) {
                p.selectedCard = card;
                p.hasVoted = true;
                Logger_1.logger.info('[SELECT_CARD] Vote recorded for:', { odId, card });
                io.to(roomId).emit('vote_update', { odId, hasVoted: true });
            }
            else {
                Logger_1.logger.error('[SELECT_CARD] Participant NOT FOUND with odId:', odId);
            }
        });
        ...
        socket.on('assign_points', async ({ roomId, issueKey, points }) => {
            ...
            if (room) {
                ...
                if (cached) {
                    if (cached.data.issueType === 'Bug')
                        cached.data.timeEstimate = String(points);
                    else
                        cached.data.currentPoints = Number(points);
                }
                await JiraService_1.jiraService.updateIssuePoints(issueKey, points, 'Story'); // Simplification
                // Re-emit issue_changed to show new points
                if (room.currentIssue && room.currentIssue.issueKey === issueKey) {
                    if (room.currentIssue.issueType === 'Bug')
                        room.currentIssue.timeEstimate = String(points);
                    else
                        room.currentIssue.currentPoints = Number(points);
            ...
            store_1.rooms.forEach((room) => {
                ...
                if (foundodId) {
                    ...
                    if (p) {
                        ...
                        Logger_1.logger.info('User removed from room (disconnected)', { odId: foundodId, roomId: room.roomId });
...
function emitRoomUpdate(io, room) {
    Logger_1.logger.info('[SOCKET] emitRoomUpdate sending to', room.roomId);
